...
class Stylization(object):
    def __init__(self, layer='r31', alpha=[0.5], remove_stochastic=True, prob=0.25, pseudo1=True, Noise=True, std=1.,mean=0.):
        self.net = StyleAugmentation(layer, alpha, remove_stochastic, prob, pseudo1, Noise, std, mean).cuda() # CUDA is available
    ...
    def __call__(self, x):
        with torch.no_grad():
            return self.net(x[None,:].cuda())[0].detach().cpu()
        # with torch.no_grad():
